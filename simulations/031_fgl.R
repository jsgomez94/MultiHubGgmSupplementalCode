...
## GGL.
cv.fgl = function(X, K, p, n, penalize.diagonal = TRUE,
  ...
  .rholist2 = .rhos[[1]] * 10^seq(from = range, to = 0, length.out = rholength2)
  ...
  for(.ind1 in 1:rholength1){
    .ggl.temp = JGL(Y = .X, penalty = "fused", 
  ...
  .opt.model = JGL(Y = .X, penalty = "fused", 
...
## pathway solution.
pathway.fgl = function(X, K, p, n, penalize.diagonal = TRUE,
                  range = -5, rholength1 = 20, rholength2 = 20,
                  maxit = 10000, threshold = 10^(-7),
                  normalize = TRUE, AIC = TRUE){
  ...
  print(.evalmeasure)
  .opt.index = which(.evalmeasure == min(.evalmeasure), arr = TRUE)
...
if(examples){
  ...
  BICpathway = pathway.fgl(X = X, K = 3, p = p, n = n, 
                           penalize.diagonal = TRUE, range = -5, 
                           rholength1 = 5, rholength2 = 5, 
                           maxit = 10, threshold = 10^(-7), 
                           normalize = TRUE, AIC = TRUE)
  AICpathway <- pathway.fgl(X = X, K = 3, p = p, n = n, 
                            penalize.diagonal = TRUE, range = -5, 
                            rholength1 = 5, rholength2 = 5, 
                            maxit = 10, threshold = 10^(-7), 
                            normalize = TRUE, AIC = TRUE)

    ## BICpathway$opt.models:               List of length rholength1 containint GGL models and tuning parameters.
